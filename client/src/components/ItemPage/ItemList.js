...
function ItemList() {
    ...
    useEffect(() => {
        fetch('https://ddragon.leagueoflegends.com/cdn/13.8.1/data/en_US/item.json')
            ...
            .then(data => {
                ...
                const itemsArray = Object.values(itemsData).filter(item => item.maps["11"] || item.maps["12"]);
                setItems(itemsArray);
            })
    ...
    return (
        ...
        <div className="container mx-auto py-4">
            <h1 className="text-2xl font-bold mb-4 text-center">LoL Items</h1>
                    ...
                    <div key={item.image.full} className="relative group">
                        <Link to={`/item/${item.image.full.split('.')[0]}`}>
                            <img
                                ...
                                alt={item.name}
                                className="w-16 h-16 mx-auto rounded-lg p-1 border border-gray-300 hover:shadow-md hover:border-yellow-500 cursor-pointer transform hover:scale-110 transition-transform"
                            />
