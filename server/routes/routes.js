...
router.get('/:region/summoner/:puuid/matchhistory', async (req, res) => {
  ...
  router.get('/models/champion', async (req, res) => {
    try {
        // Check if champions data exists in the database
        let champions = await Champion.find();

        // If not in the database, fetch from Riot API
        if (!champions.length) {
            const response = await axios.get('https://ddragon.leagueoflegends.com/cdn/13.18.1/data/en_US/champion.json');
            champions = response.data.data;
            await Champion.insertMany(Object.values(champions));
        }
